...
void b2MotorJoint::SolveVelocityConstraints(const b2SolverData& data)
{
	...
	// Solve linear friction
	{
		...
		b2Vec2 oldImpulse = 0.5 * m_linearImpulse;
		m_linearImpulse += impulse;

		float32 maxImpulse = h * m_maxForce;
		float32 minImpulse = h * m_minForce;

		if (m_linearImpulse.LengthSquared() > maxImpulse * maxImpulse)
		{
			m_linearImpulse.Normalize();
			m_linearImpulse *= maxImpulse;
		} else if(m_linearImpulse.LengthSquared() < minImpulse * minImpulse) {
			m_linearImpulse.Normalize();
			m_linearImpulse *= minImpulse;
		}
...
void b2MotorJoint::SetMinForce(float32 force)
{
	b2Assert(b2IsValid(force) && force >= 0.0f);
	m_minForce = force;
}
...
float32 b2MotorJoint::GetMinForce() const
{
	return m_minForce;
}
