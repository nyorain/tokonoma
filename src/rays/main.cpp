...
public:
	static constexpr auto sampleCount = 800u;
...
public:
	...
	void initScene(vk::CommandBuffer cb) {
		...
		std::initializer_list<Light> lights = {
			{fac * tkn::blackbody(3000), 0.1f, {-1.f, 1.f}},
			// {fac * tkn::blackbody(5500), 0.1f, {2.3f, 2.f}},
			// {fac * tkn::blackbody(5500), 0.1f, {-2.f, -2.f}},
			// {fac * tkn::blackbody(5000), 0.3f, {-2.f, 2.f}},
			// {fac * tkn::blackbody(7000), 0.1f, {2.f, -1.f}},
		};

		std::initializer_list<Material> mats {
			{{1.f, 1.f, 1.f}, 1.f, 0.f}, // white rough
			{{1.f, 0.8f, 0.8f}, 0.2f, 0.f}, // red shiny
			{{0.8f, 1.0f, 0.8f}, 0.2f, 1.f}, // green metal
			{{0.8f, 0.8f, 1.0f}, 0.05f, 0.f}, // blue mirror
			{{1.0f, 1.0f, 0.8f}, 0.05f, 1.f}, // yellow metal mirror
	...
	void initGfx() {
		...
		vpp::GraphicsPipelineInfo pipeInfo(renderPass(), gfx_.pipeLayout, {{{
		...
		pipeInfo.rasterization.lineWidth = 2.f;
	...
	void beforeRender(vk::CommandBuffer cb) override {
		...
		vk::cmdDispatch(cb, nLights_, sampleCount, 1);
